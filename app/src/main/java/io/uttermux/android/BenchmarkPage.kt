...
@Composable fun BenchmarkPage(focusArtifact:String="",onStatus:(String)->Unit){
    ...
    LazyColumn(Modifier.fillMaxSize().padding(12.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
        ...
        items(voices,key={runner.artifactId(it)}){voice->val artifact=runner.artifactId(voice);val latest=runner.latest(artifact);val latestWinner=latest?.optJSONObject("winner");val preview by PreviewController.state.collectAsState();val previewActive=preview.voiceId==voice.id&&preview.phase in setOf("loading","playing");var settingsOpen by remember(artifact){mutableStateOf(false)};Card{Column(Modifier.fillMaxWidth().padding(12.dp),verticalArrangement=Arrangement.spacedBy(6.dp)){
            ...
            Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(8.dp)){Button(onClick={val signal=AtomicBoolean();cancel=signal;active=artifact;scope.launch{runCatching{withContext(Dispatchers.Default){runner.run(voice,signal){message->scope.launch{progress=message}}}}.onSuccess{pending=it;progress="Review ${it.classification}: ${"%.3f".format(it.winner.medianRtf)} RTF, ${it.winner.threads} threads"}.onFailure{progress=if(it is InterruptedException)"Benchmark cancelled" else "Benchmark failed: ${it.message}"};active=null;cancel=null;revision++}},modifier=Modifier.weight(1f),enabled=active==null){Text(if(active==artifact)"Running…" else "Benchmark")};OutlinedButton(onClick={if(previewActive)PreviewController.stop()else scope.launch{runCatching{PreviewController.play(voice.id){signal->app.router.synthesizeExact(voice.id,"UtterMux variant comparison preview.",voice.locale.toLanguageTag(),1f,signal)}}.onFailure{onStatus("Preview failed: ${it.message}")}}},modifier=Modifier.weight(1f),enabled=active==null){Text(if(previewActive)"Stop" else "Preview")}}
            TextButton(onClick={settingsOpen=!settingsOpen},modifier=Modifier.fillMaxWidth()){Text(if(settingsOpen)"Hide model settings" else "Model settings")}
