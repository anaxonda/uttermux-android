...
class AzureProvider(private val secure:SecureStore):TtsProvider {
    ...
    override fun isAvailable(voice:VoiceRecord)=secure.get("azure").isNotBlank()&&secure.get("azure_region").isNotBlank()
    private fun base()=secure.get("azure_endpoint").ifBlank{"https://${secure.get("azure_region")}.tts.speech.microsoft.com"}.trimEnd('/')
    ...
    override fun refresh(){
        ...
        val array=JSONArray(String(HttpAudio.get("${base()}/cognitiveservices/voices/list",mapOf("Ocp-Apim-Subscription-Key" to key))))
    ...
    override fun stream(session:PreparedSession,text:String,speed:Float,pitch:Float,cancelled:AtomicBoolean,emit:(AudioChunk)->Boolean){
        ...
        HttpAudio.postStreamRaw("${base()}/cognitiveservices/v1",ssml.toByteArray(),"application/ssml+xml",mapOf("Ocp-Apim-Subscription-Key" to key,"X-Microsoft-OutputFormat" to "raw-24khz-16bit-mono-pcm","User-Agent" to "UtterMux"),cancelled){emit(AudioChunk(it,24000,range,sequence++))}
    }
}
