...
object HttpAudio {
    ...
    fun postRaw(url:String,body:ByteArray,contentType:String,headers:Map<String,String>,cancelled:AtomicBoolean?=null):ByteArray {
        val builder=Request.Builder().url(url).post(body.toRequestBody(contentType.toMediaType()));headers.forEach(builder::header)
        return executeBytes(builder.build(),cancelled)
    }
    ...
    fun executeBytes(request:Request,cancelled:AtomicBoolean?=null):ByteArray {
        val call=client.newCall(request);val result=AtomicReference<Result<ByteArray>>();val done=CountDownLatch(1)
        call.enqueue(object:okhttp3.Callback{
            override fun onFailure(call:okhttp3.Call,error:java.io.IOException){result.set(Result.failure(error));done.countDown()}
            override fun onResponse(call:okhttp3.Call,response:okhttp3.Response)=response.use{
                result.set(runCatching{if(!it.isSuccessful)throw ProviderException(it.code,it.body.string().take(1000));it.body.bytes()});done.countDown()
            }
        })
        while(!done.await(50,TimeUnit.MILLISECONDS))if(cancelled?.get()==true){call.cancel();throw InterruptedException()}
        if(cancelled?.get()==true)throw InterruptedException()
        return result.get().getOrThrow()
    }

    fun executeStream(request:Request,cancelled:AtomicBoolean,emit:(ByteArray)->Boolean){
        val call=client.newCall(request);val finished=AtomicBoolean()
        Thread({while(!finished.get()&&!cancelled.get())Thread.sleep(25);if(cancelled.get())call.cancel()},"uttermux-http-cancel").apply{isDaemon=true;start()}
        try{call.execute().use{response->
            if(!response.isSuccessful)throw ProviderException(response.code,response.body.string().take(1000))
            val input=response.body.byteStream();val buffer=ByteArray(16*1024);var carry=-1
            while(!cancelled.get()){
                val start=if(carry>=0)1 else 0;val count=input.read(buffer,start,buffer.size-start);if(count<0)break
                if(carry>=0)buffer[0]=carry.toByte();val total=count+start;val even=total-total%2
                if(even>0&&!emit(buffer.copyOf(even)))return
                carry=if(total%2==1)buffer[total-1].toInt() and 255 else -1
            }
            if(cancelled.get())throw InterruptedException()
