...
object TextSegmenter {
    fun split(text:String,firstTarget:Int=120,nextTarget:Int=260,maxChars:Int=360):List<TextSegment> {
        ...
        while(start<text.length) {
            ...
            var end=findBoundary(text,start,desired,hardEnd)
    ...
    private fun findBoundary(text:String,start:Int,desired:Int,hardEnd:Int):Int {
        if(hardEnd==text.length)return hardEnd
        for(i in hardEnd downTo desired)if(text[i-1] in ".!?;:\n")return i
        for(i in hardEnd downTo desired)if(text[i-1].isWhitespace())return i
        for(i in desired downTo start+1)if(text[i-1] in ".!?;:\n"||text[i-1].isWhitespace())return i
        return hardEnd
    }
}
