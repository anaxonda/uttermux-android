...
class KoReaderServerService:Service(){
    ...
    private fun voices():String{val groups=JSONObject();app.router.availableVoices.forEach{voice->val key=voice.locale.toLanguageTag().replace('-','_');val array=groups.optJSONArray(key)?:org.json.JSONArray().also{groups.put(key,it)};array.put(voice.id)};return groups.toString()}
